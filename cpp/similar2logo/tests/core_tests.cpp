...
void testMathUtil() {
  ...
  assert(std::abs(s2l::tools::MathUtil::normalizeAngle(3 * M_PI) - M_PI) <
         1e-9);

  // Test angleDifference
  assert(std::abs(s2l::tools::MathUtil::angleDifference(0.0, M_PI / 2) -
                  M_PI / 2) < 1e-9);
...
void testAgentCategory() {
  ...
  // Constructor
  mk::AgentCategory cat("test_category");
  assert(cat.toString() == "test_category");

  // Copy constructor
  mk::AgentCategory cat2(cat);
  assert(cat2.toString() == "test_category");

  // Comparison
  mk::AgentCategory cat3("test_category");
  mk::AgentCategory cat4("other_category");
...
void testSituatedEntity() {
  ...
  // SituatedEntity is abstract, so we test it via TurtlePLSInLogo
  // which inherits from it
  s2l::tools::Point2D loc(10.0, 20.0);
  auto turtle = std::make_shared<s2l::model::environment::TurtlePLSInLogo>(
      loc, 0.0, 1.0, 0.0, true, "blue");

  assert(turtle->getLocation() == loc);

  // Set location (inherited from SituatedEntity)
